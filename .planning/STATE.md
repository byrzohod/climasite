...
- **Last checkpoint**: 2026-06-29 (B-018/B-020 frontend load-error states in PR `fix/frontend-load-error-states` — `/acceptance` PASS live + 2-round Codex council clean; the backend quick-wins batch #88 + B-002 #87 already merged)
...
## Current position
- **Phase**: maintenance / incremental hardening, working the triaged external-review register. **MERGED to `main`** (tip `28acac9`): PAY-IDEM (#84), triage (#85), SEC-05/B-011 (#86), **B-002 admin price-inversion (#87)**, **backend quick-wins batch B-007/B-008/B-034/B-036/B-055 (#88)**. Both real Highs (B-011, B-002) DONE; the 5 backend quick wins DONE.
- **B-018/B-020 frontend load-error states are in PR `fix/frontend-load-error-states`** — a cart/orders load FAILURE was rendered as a fake "empty" state; now each surface shows an accessible (`role="alert"`) error + **Retry**. B-018: `orders.component` `loadError` signal + error/retry branch. B-020: `CartService` gained a dedicated `_loadFailed` (separate from transient mutation `_error`, so a failed quantity-update doesn't blank the cart) + preserves the loaded cart; the **cart page, checkout, AND mini-cart drawer** render error+Retry before isEmpty. **Codex council 2 rounds** — round 1 caught the checkout+mini-cart fake-empty gap (Medium) + a11y live-region (Low) → fixed; round 2 clean (its "generic error" Low → resolved via the `_loadFailed` split). New i18n `cart.errors.loadFailed` + `account.orders.errors.loadListFailed` (en/bg/de). **`/acceptance` PASS** live (cart/orders/checkout error+Retry, not empty; Retry recovers). FE suite 1749 green. Awaiting CI → squash-merge.
- The **external review register** now has both real Highs + B-007/B-008/B-034/B-036/B-055 + B-018/B-020 marked DONE. Follow-ups in `PRIORITIZED_BACKLOG.md`: FOUND-B002-orphans, FOUND-B002-noimage, FOUND-QW-admin-pagination, **FOUND-loaderr-race** (latest-request guard).
