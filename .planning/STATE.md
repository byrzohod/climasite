...
## Current position
- **Phase**: maintenance / incremental hardening. External-review register CLEARED; **B-044 SEO fully merged** (#92 `819fb18` + #93 `ae082df`).
- **B-039 per-voter Q&A vote ledger — in PR `feature/b-039-qa-vote-ledger` (awaiting CI → squash-merge).** Replaces the inflatable anonymous Q&A voting with a `ReviewVote`-style ledger: `ProductQuestionVote`/`ProductAnswerVote` + `UNIQUE(target,user)` (expand migration `AddQaVoteLedgers`), `[Authorize]`+strict-rate-limit, self-vote 400 / non-approved 404, **toggle** (answer flip), DTO `hasVotedHelpful`/`userVoteHelpful` (batched) → UI uses authoritative server state (localStorage tracking removed; per-id overlap guard). **Concurrency-hardened**: atomic `ExecuteUpdate` count deltas gated on rows-affected (`ON CONFLICT`/conditional `DELETE`/`UPDATE WHERE is_helpful=@from`) with read+decision **outside** the retrying tx (idempotent under `EnableRetryOnFailure`, break-probe-proven). GDPR hard-delete (counts = anonymized aggregates). 3 design council rounds + backend REWORK→CLEAN + combined APPROVE-WITH-CHANGES→fixed; `/acceptance` PASS at `.planning/acceptance/B-039-qa-vote-ledger.md`. **Gotcha this unit: CI 'Lint & Format' = `dotnet format ClimaSite.NoE2E.slnf --verify-no-changes` (default severity) — do NOT run `--severity info` (it reformats the WHOLE repo); it flagged a mid-fluent-chain comment + an EF-generated UTF-8 BOM on the new migration `.cs` (strip it; the snapshot legitimately keeps its BOM as auto-generated).**
- **NEXT queue (owner picks)**: B-016 (recommendation spec-key aliasing), a11y batch B-001/B-014/B-042, deploy-hardening F3/OPS-07/OPS-04 (+Railway, owner-gated), or the B-039 follow-up (per-user rate-limit partition). Only acknowledged Low left = notifications `summary` `recentCount` (intentional "recent N").
