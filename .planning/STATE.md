...
- **Last checkpoint**: 2026-07-01 (**B-038 Q&A answered-state + B-039-followup per-user rate-limit** — in PR `feature/b-038-b-039-qa-answered-state-ratelimit`, design+diff councils clean after fixes, `/acceptance` PASS, awaiting CI. **All prior items MERGED to `main` tip `bff4277`**: B-044 SEO #92/#93, B-039 #94, a11y batch #95, B-016 #96.)
...
## Current position
- **Phase**: maintenance / incremental hardening. External-review register CLEARED. `main` tip = **`bff4277`** (B-016 #96). All merged: B-044 SEO #92/#93, B-039 Q&A vote ledger #94, a11y batch #95, B-016 HVAC spec resolver #96.
- **B-038 + B-039-followup — in PR `feature/b-038-b-039-qa-answered-state-ratelimit`** (awaiting CI → squash-merge). **B-038**: a still-`Pending` answer flagged its question "answered" (hidden from the unanswered queue, 0 answers shown) — fixed by deriving answered-state solely from approved-answer existence (removed `MarkAsAnswered`/`SetStatus` side-effect → idempotent `RefreshAnsweredState`; `ModerateAnswerCommand` reconciles from the loaded graph; public + admin reads derive filter/count/timestamp from `Answers.Any(Approved)` → legacy dirty data heals at read time, no migration). **B-039-followup**: split `strict` (IP-only, anonymous payment/contact/installation) from new `strict-user` (per-user, IP-fallback) on the `[Authorize]` vote endpoints; moved `UseAuthentication` before `UseRateLimiter`; config-gated the limiter (`RateLimiting:Enabled`) to enable a two-user partition integration test. Design council caught 3 Highs (middleware order, anonymous-endpoint split, read-side derivation) + diff council 2 Med/3 Low — all fixed → re-councilled. `/acceptance` PASS at `.planning/acceptance/B-038-B-039-qa-hardening.md`. All break-probe-verified. Core 430 / Application 968 / Api 479 green; `dotnet format` clean.
...
- **NEXT queue (owner picks)**: deploy-hardening F3/OPS-07/OPS-04 (+Railway, owner-gated) · SEC-12 dependency upgrades (incl. the Microsoft.OpenApi/Swashbuckle bump the B-039 allowlist defers) · inventory reservations (oversell window). B-038 + B-039-followup now DONE (this PR). Only acknowledged Low left = notifications `summary` `recentCount` (intentional "recent N").
