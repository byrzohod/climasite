...
- **Last checkpoint**: 2026-07-02 (**INV-01 Wave B (bank-transfer hold-with-expiry) CODE-COMPLETE + council-clean + `/acceptance` PASS**, on branch `feature/inv-01-b-bank-transfer-hold`, opening PR — the LAST slice. A0 #98 + A1 #99 + A2 #100 + A3 #101 merged (`main` tip `51fd8bf`). B: bank order RESERVES (no hard-decrement) at create, admin mark-paid CONSUMES, unpaid→sweeper AUTO-CANCELS + releases; all order-status transitions (mark-paid/cancel/sweep) serialize on an **order-row FOR UPDATE lock** (before variant locks); consume on ANY →Paid, all-or-nothing; `bank + paymentIntentId` rejected. Diff R1 REWORK (4 lifecycle gaps) → R2 REWORK (webhook bank+PI) → clean. Core 430/App 1045/Api 526, format clean. **Live /acceptance: bank order held not decremented; unpaid → sweeper cancelled in <20s, stock never leaked; bank+PI → 400.** After merge, INV-01 is COMPLETE (all 5 waves).
  - **Follow-up filed (not a B blocker, pre-INV-01, all order types):** cancelling a PAID order via the admin STATUS endpoint (`UpdateOrderStatusCommand`→Cancelled) does NOT restock — only `CancelOrderCommand` restocks a consumed order. Route the status endpoint's terminal transitions through the same restock/release logic.)
...
## Current position
- **Phase**: feature build — **INV-01 checkout stock-reservations** (owner-picked from the NEXT queue 2026-07-01). `main` tip = **`114e610`** (#97 B-038 + B-039-followup). All prior items merged (B-016 #96, a11y #95, B-039 #94, B-044 SEO #92/#93).
- **INV-01 — unit-plan APPROVED (`.planning/units/INV-01-checkout-reservations/unit-plan.md`), council-CONVERGED** (design R1 REWORK → R2 REWORK-narrow → **R3 CONVERGED**; Codex gpt-5.5@xhigh + Claude 3-lens). **Reframe:** the DB never oversells (atomic `TryDecrementVariantStockAsync`); the real defects are **charge-then-refund the losing last-unit buyer** (card charged client-side BEFORE the order/stock check), a **bank-transfer stock leak** (hard-decrement, no restock if unpaid), and **dishonest availability** (raw stock; `ReservedQuantity` DTO vestigial). **Owner decisions:** reserve at checkout-start (hold BEFORE the charge; 20-min TTL) · `stock_reservations` ledger + denormalized `reserved_quantity` counter · enforce at checkout + honest availability on PDP/cart only · bank-transfer = hold-with-expiry + auto-restock · **FULL anti-grief (server-minted signed guest token)**.
  - **THE MECHANISM (council-forged):** `SELECT … FOR UPDATE` on the variant row = the single serialization primitive for all hold mutations (P1 reserve reconcile-to-target / P2 consume decrement-first / P3 release+expiry-CAS / R reconciler); **`reserved_quantity` honored by EVERY stock writer** (reserved-aware `TryDecrementVariantStockAsync`) so nothing drains a held unit; **clock-independent counter** (expired-but-unswept holds still count+protect; sweeper is sole releaser); **refund-idempotency at every call-site** (closes a real concurrent-double-submit "shipped-for-free" hole) + deterministic-order.Id lookup; ascending-`variant_id` lock ordering; drift self-heal reconciler. Concurrency proven by **mandatory Testcontainers break-probes** (over-reserve / oversell-via-sweeper / sibling-refund / reverse-order-deadlock) that MUST fail with the fix removed.
  - **3 gated PRs:** **A0** signed guest-session token, shipped DARK — **✅ MERGED `02e700b` (#98)** (cookie minted/validated/published, NOT yet authoritative for cart-keying; `CartController` byte-identical to main) → **A** = guest-identity switch (flip cart+checkout→cookie + legacy-cart migration + legacy-reject) **+ reservations core** (ledger + `reserved_quantity` + FOR-UPDATE P1/P2/P3 + reserved-aware `TryDecrementVariantStockAsync` + sweeper + drift reconciler + order-create idempotency + PDP/cart availability + admin reserved-awareness) → **B** bank-transfer hold-with-expiry. A0 building blocks now on `main`: `IGuestSessionTokenService`/`GuestSessionTokenService` (3-part `{id}.{exp}.{sig}` HMAC), `GuestSessionMiddleware`+`UseGuestSession` (mints on `/api/cart` only, publishes `IGuestSessionAccessor`), `GuestSessionMintLimiter`. Wave A wires `IGuestSessionAccessor` into cart/payments/orders controllers + adds `/api/payments`,`/api/orders` to `MintPathPrefixes` + re-adds `AllowLegacyId`.
- **NEXT ACTION:** Wave A0 done (dark cookie infra) → PR open, get 6 green CI → squash-merge. **Then Wave A** = the guest-identity SWITCH (flip cart+checkout to the cookie + legacy-cart migration + legacy-reject; A0 shipped the cookie dark) **+ the reservations core** (ledger + `reserved_quantity` + FOR-UPDATE P1/P2/P3 + reserved-aware decrement + sweeper + drift reconciler + order-create idempotency + PDP/cart availability). Then Wave B = bank-transfer hold-with-expiry.
- **Deferred NEXT queue (after INV-01):** deploy-hardening F3/OPS-07/OPS-04 (+Railway, owner-gated) · SEC-12 dependency upgrades (Microsoft.OpenApi/Swashbuckle). Only acknowledged Low left = notifications `summary` `recentCount`.
