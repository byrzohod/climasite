...
- [ ] **P1** Consolidate deployment config: three disagreeing Railway configs (root `railway.toml` → Dockerfile.web, root `railway.api.toml` → Dockerfile.api, `src/ClimaSite.Api/railway.toml` → "Dockerfile") and drifting duplicate Dockerfiles (`src/ClimaSite.Web/Dockerfile` vs root `Dockerfile.web`; stale `src/ClimaSite.Web/nginx.conf` lacking the `/api` proxy block — a dead edit-trap). Keep one canonical Dockerfile + one Railway config per service; delete the rest; document the service→config mapping. (`_review/devops.md` #2)
