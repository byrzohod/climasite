...
- **FOUND-B002-orphans (Low, S — latent/dead-code):** Found while fixing B-002 (3-lens verification). Two **unwired/orphan** components use the OPPOSITE sale convention from the `ProductPricing` contract — they treat `salePrice` as the *discounted/lower* price (`salePrice < price` → render `salePrice` prominent): `frequently-bought.component.ts` (guards at :35,:68; effective-price math at :440,:445,:463,:468) and `product-variants.component.ts` (:49; price-range at :338). Zero `<app-frequently-bought>`/`<app-product-variants>` template usages today, so **not shipped** — but if either is ever wired to the standard backend DTOs (where `salePrice` = the higher original), a real discount silently never displays and totals use full price. **Fix:** flip the predicate to `salePrice > price`, render `price` (current) prominent + `salePrice` (original) struck, and compute totals off `price` — OR delete if confirmed dead. (B-002 itself is the admin-list slice; this is the same class of bug in dead code.)
- **FOUND-B002-noimage (Low, XS — cosmetic):** `assets/images/no-image.svg` (the placeholder fallback in admin product list `:169` and elsewhere) is **missing from `src/assets/`**, so every imageless product 404s a broken thumbnail. Pre-existing, unrelated to B-002 (surfaced by its `/acceptance` run). **Fix:** add the SVG placeholder asset, or change the fallback to an existing asset; grep for other `no-image.svg` references.
...
- **FOUND-loaderr-race (Low, S):** Found in the B-018/B-020 council. `loadOrders()`/`loadCart()` have no latest-request guard — an older failing request can resolve after a newer success and re-show the error over valid data. Pre-existing `.subscribe()` pattern. **Fix:** a request sequence id or `switchMap` so only the latest load updates the error/loadFailed signals.
...
- **FOUND-QW-admin-pagination (Low, S):** Found in the QW-backend-batch council (round 2). Admin list endpoints (`AdminProductsController` → `GetAdminProductsQuery`, plus admin orders/customers/questions/reviews/inventory/notifications lists) still pass unbounded `pageNumber`/`pageSize` straight into their own `Skip`/`Take` + `Math.Ceiling(count/pageSize)`, so `pageSize=0` → div-by-zero 500 and a huge `pageNumber` → overflow. **Auth-gated** (an admin can only break their own request), so deferred from QW-backend-batch (which clamped all *anonymous* public endpoints). **Fix:** apply `QueryBounds.PageNumber/PageSize` at those admin controllers (NOT the dashboard `count` endpoints — different semantics).
