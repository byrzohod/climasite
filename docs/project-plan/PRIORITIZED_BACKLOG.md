...
- **B-016 (Medium, M) ✅ DONE 2026-07-01:** Recommendation scoring / facets read canonical spec keys the display-key catalog never emits → flat fallback. Fixed with a shared alias-aware **`HvacSpecResolver`** (ordered aliases + per-key typed parsers + unit-stripping/rejection, CLR + JSONB `JsonElement`) wired into the scoring service, recommendation handler, and both facet extractors; `DataSeeder` gains the machine-only canonical fields; the public DTO strips them from the PDP. A **nested defect** (the `90/110/140` BTU/m² model was ~2–3× too low → real ratings scored 0 for normal rooms) was recalibrated to `A=200/B=250/C=320` (owner-approved) across the BE scoring + FE preview, default wizard area `24→35`. Also fixed the BG/DE FAQ "per square metre" mistranslation. No migration; write-boundary rewriting deliberately dropped (resolver-at-read is the enforcement). Design + diff councils clean (re-council: no findings); `/acceptance` PASS (size-matched recs + PDP strip + FE preview live). DataSeeder-shape scoring test + display-only JSONB integration test + facet merge/precedence tests + break-probe. Core 428 / Application 965 / Api 470 / FE 1788 green. (B-016-spec-aliasing.)
- **B-038 (Medium, S) ✅ DONE 2026-07-01:** A still-`Pending` answer flagged its question "answered" (`AnswerQuestionCommand` called `MarkAsAnswered()` on submission) while the read side only counts/shows approved answers → question hidden from the unanswered queue with `AnswerCount` 0. Fixed by deriving answered-state **solely from approved-answer existence**: removed `MarkAsAnswered()` + the `SetStatus` side-effect, added idempotent `ProductQuestion.RefreshAnsweredState()` (set on first approval, clear on last un-approval), `ModerateAnswerCommand` loads sibling answers + reconciles in-memory, and both the public `GetProductQuestionsQuery` and admin `GetPendingModerationQuery` derive filter/count/timestamp from `Answers.Any(Status==Approved)` (heals legacy dirty data at read time, no migration). Documented residual: simultaneous multi-admin moderation of one question can transiently drift the persisted `AnsweredAt` (user-invisible, self-corrects; no optimistic-concurrency token for a human-paced path). Shipped with the **B-039 follow-up** (per-user `strict-user` rate-limit partition — the `strict` policy split so authenticated voters no longer share one NAT/CGNAT IP bucket; required moving `UseAuthentication` before `UseRateLimiter`). Design + diff councils clean after fixes (re-councilled); `/acceptance` PASS (pending→approve→reject self-heal + live 429). Break-probe-verified incl. a two-user rate-limit partition integration test. Core 430 / Application 968 / Api 479 green. (B-038-B-039-qa-hardening.)
