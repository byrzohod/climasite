...
**Date:** 2026-06-11
**Status:** Authoritative going forward. Where this document disagrees with CLAUDE.md, AGENTS.md, or `.claude/skills/release/SKILL.md`, **this document is correct** — those files contain verified errors (wrong E2E commands, wrong API port, direct-push-to-main instruction) documented in `docs/project-plan/_review/devops.md`, `_review/testing.md`, and `_review/docs.md`, and should be corrected to match this file.
...
- **Stripe:** the code reads **config-section keys only** (`Stripe:SecretKey` in `StripePaymentService.cs`, `Stripe:WebhookSecret` in `WebhooksController.cs`). The flat `STRIPE_SECRET_KEY` env var documented in CLAUDE.md **does not reach the code** — use `Stripe__SecretKey` / `Stripe__WebhookSecret` (double underscore) or edit `appsettings.Development.json`. `appsettings.json` ships non-empty **dummy** keys (`sk_test_51DummyKey…`), so checkout will silently "work" until Stripe rejects the call. There is **no documented local Stripe test-mode setup** (keys, webhook forwarding via `stripe listen`) — gap, P2 (`_review/devops.md` finding 10).
...
- `main` = integration branch (and future deploy branch). **Never push directly to main** — CLAUDE.md's "Post-Implementation Workflow" step 4 (`git push origin main`) is **wrong and superseded**; recent practice (PRs #1–#3) and this document mandate PRs. Note: branch protection is **not yet enabled** on GitHub (verified 404 on the protection API) — enabling it is a P1 action; until then the rule is by discipline.
...
1. **Correction to `.claude/skills/release/SKILL.md` pre-flight step 3:** the local test command it lists ends with `npx playwright test` — that command does not exist here. Use the section 4 suite of this document instead. (The skill file should be edited to match.)
2. **Procedure (per the skill, valid):** confirm green CI on `main` → decide semver bump from conventional commits (`feat!:`→MAJOR, `feat:`→MINOR, `fix:`/`chore:`→PATCH) → roll `[Unreleased]` into a versioned CHANGELOG section → release notes in `docs/releases/v<X.Y.Z>.md` (directory doesn't exist yet — create it) → `chore(release): v<X.Y.Z>` commit via PR → annotated tag `v<X.Y.Z>` → `git push origin main --follow-tags`.
