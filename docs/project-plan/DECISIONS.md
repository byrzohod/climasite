...
- **Alternatives considered:** Not recorded.
- **Consequences:** Reasonable topology. _(UPDATE 2026-06-27, OPS-03:_ the duplicate edit-traps are **resolved** — `src/ClimaSite.Api/railway.toml`, `src/ClimaSite.Api/Dockerfile`, `src/ClimaSite.Web/Dockerfile`, and the stale `src/ClimaSite.Web/nginx.conf` were deleted; the canonical set is root `railway.toml`→`Dockerfile.web` + `railway.api.toml`→`Dockerfile.api`, with `nginx.conf.template` for the web image. The service→config mapping is documented in `docs/runbooks/deploy.md`.)_ The deploy workflow (`.github/workflows/deploy.yml`) is still TODO (owner-gated). **Owner must confirm** (devops open questions): which services exist in the Railway dashboard, whether auto-deploy from main is enabled, and whether Postgres backups are on. Also note the P0 rider: production startup currently seeds `admin@climasite.local`/`Admin123!` (`_review/devops.md` finding 1) — that was never a decision, it is a defect (rotate on first deploy — OPS-08 §5).
