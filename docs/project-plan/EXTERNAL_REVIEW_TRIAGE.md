...
| B-007 ✅ FIXED 2026-06-29 | Order email links include a literal `$` before the order ID | C | Medium (council ↓ from High) | — (new, NEW-BUG) | DONE: single `EmailService.BuildOrderUrl` helper (no `$`) used by both order emails; unit-tested. |
| B-008 ✅ FIXED 2026-06-29 (residual) | Raw `ArgumentException.Message` echoed to clients | P | Medium | ARCH-04 / SR-18 (residual) | DONE: ArgumentException → generic "Invalid request" + dropped from the `detail` whitelist; middleware unit-tested. **High-half (guest-token "throw") was a FALSE_POSITIVE.** |
...
| B-018 ✅ FIXED 2026-06-29 | Account order load failures shown as empty history | C | Medium | UX-05 | DONE: `loadError` signal + error/retry branch before the empty-state; live-proven (error+Retry, not "no orders"). |
...
| B-020 ✅ FIXED 2026-06-29 | Cart load failures converted into an empty-cart state | C | Medium | — (new, NEW-UX; UX-02 is wishlist) | DONE: `loadCart` sets `_error` + preserves the cart; error+Retry branch on the cart page, checkout, AND mini-cart (council caught the latter two). Live-proven. |
...
| B-034 ✅ FIXED 2026-06-29 | Installation request endpoint lacks the strict rate limit | C | Medium | — (new, NEW-SEC) | DONE: `[EnableRateLimiting("strict")]` added; live-proven 429 after the 5/min budget. |
| B-036 ✅ FIXED 2026-06-29 | Pagination query bounds not enforced on public endpoints | C | Medium | PERF-02 | DONE: `QueryBounds` helper (PageNumber 1..100000, PageSize 1..100, Count 1..24) at the edge + `PaginatedList` floor; live-proven cap@100 + no 500 on garbage. |
