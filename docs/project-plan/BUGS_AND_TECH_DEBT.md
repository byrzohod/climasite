...
## 8. Resolved after the snapshot — live-bug sweep (2026-06-20, PRs #35 & #36)

These defects were found by **driving the running app** (node Playwright + `document.elementFromPoint`) after the owner reported the cart/checkout and reviews were visibly broken even though all 1,751 non-E2E tests and CI were green — none were in the original register, and the existing E2E suite never opened the mini-cart and *clicked* its actions, so they shipped undetected. All are **FIXED** and verified live.

| ID | Title | Root cause | Fix |
|----|-------|-----------|-----|
| BUG-19 | Mini-cart drawer fully non-interactive (clicks hit the backdrop and close it) — the "can't click checkout / can't buy" report | **Duplicate z-index scale**: `styles/_spacing.scss` emitted a second `$z-index` map into `:root` and, because `styles.scss` `@use`s spacing *after* tokens, it clobbered `--z-modal` to **60** (< backdrop `--z-overlay` 400), so the drawer rendered below its own backdrop | #35 — removed the duplicate map; `styles/_tokens.scss` is now the **single source of truth** for `--z-*` (modal 500 > overlay 400) |
| BUG-20 | Cookie-consent banner covered the drawer's Checkout button (desktop) and the entire bottom-nav (mobile) | Hardcoded `z-index:1000; position:fixed; bottom:0` | #35 — `z-index: var(--z-banner)` (300) + `<768px` `bottom: calc(56px + safe-area)` offset above the bottom-nav |
| BUG-21 | Mobile bottom-nav punched through the open mobile menu | bottom-nav `z-index:1000` > menu `201` (both hardcoded) | #35 — moved menu/overlay/bottom-nav onto tokens (menu 500 > overlay 400 > nav 200) |
| BUG-22 | A submitted review silently vanished (no feedback, never appeared) | Created `ReviewStatus.Pending` + public query returns `Approved`-only + no success UI | #35 — **auto-approve on create** (owner decision: reviews appear immediately) + dismissible success banner |
| BUG-23 | Review/Q&A guest "Log in" CTAs 404'd | `routerLink="/auth/login"` — the real route is `/login` | #35 — corrected the 3 links to `/login` |
| BUG-24 | Even after auto-approve, a just-posted review didn't appear for ~5 min | Global `AddOutputCache(AddBasePolicy(Expire(5 min)))` served a stale (empty) reviews list; the POST evicts nothing (diagnose: plain GET stale vs `?_cb=` cache-buster fresh) | #36 — `[OutputCache(NoStore = true)]` on `ReviewsController` (same pattern as `CartController`) + an integration test that primes the cache, posts, and asserts immediacy |
| BUG-25 | Card checkout showed a generic "Payment failed" | The configured Stripe key is a placeholder, so create-intent returns 400 (config, not code) | #35 — clearer "card payments unavailable — use bank transfer" message; **card stays default; bank transfer is the working path**. *Still open (config):* card needs a real Stripe **test** key in env (not committable) |

**New complete-workflow E2E coverage** (closes the gap that let the above ship): `tests/ClimaSite.E2E/Tests/Overlays/MiniCartOverlayTests.cs` (opens the mini-cart **with the cookie banner present** and asserts via `elementFromPoint` that the drawer actions are the real topmost element + real-click navigation), `Checkout/GuestCheckoutCompletionTests.cs`, `Cart/MiniCartPurchaseTests.cs`, `Reviews/ReviewAppearsImmediatelyTests.cs`, plus the `ReviewsControllerTests` cache-freshness integration test and unit specs.

**Convention established (apply going forward):** the `--z-*` layering scale lives **only** in `styles/_tokens.scss` — never re-introduce a `$z-index` map or `--z-*` emission in `_spacing.scss` (or any file `@use`d after tokens), and prefer `var(--z-*)` over hardcoded numbers for new overlays. ~20 legacy components still hardcode `z-index:1000+`; they sit above the token layers and work, but new token-based overlays must stay mutually consistent (a stylelint rule forbidding literal `z-index` would harden this).
