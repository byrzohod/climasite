...
- `/acceptance` -- the **runtime** complement to this static-review layer: after review is clean, the exploratory/adversarial runtime acceptance gate drives the REAL running app (UI via Playwright; API/CLI for backend-only) and **must PASS** before `/trunk-merge` (no behavior/source change merges without a PASS report whose commit matches the merged tip). Static review (this skill) + runtime acceptance together gate the queue.
