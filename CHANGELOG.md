...
- **B-044 / B-048 / B-033 — SEO foundation: per-route meta + JSON-LD wiring (Wave A) and dynamic robots.txt + sitemap.xml (Wave B).** The CSR storefront previously served one generic title and no structured data to crawlers. **Wave A (frontend, #92):** a single `SeoService` head/meta authority — idempotent ` | ClimaSite`-suffixed `<title>`, meta description, self-referential `<link rel=canonical>` (query/fragment stripped, trailing-slash normalised, base from `window.location.origin`), Open Graph + Twitter cards, and `<meta name=robots>`, re-applied on language change; a custom `TitleStrategy` applies per-route `data.seo` defaults on every navigation so meta never leaks across routes, with private + filtered-list + not-found routes marked `noindex,follow`. The pre-built `StructuredDataService` is now wired into home (Organization + WebSite), product detail (Product — effective current price + variant-derived availability + absolute images), product list/category (ItemList), FAQ (FAQPage), and breadcrumb trails (BreadcrumbList) with stable per-type ids cleared on `NavigationStart` so they don't leak (**B-048** — the dead component-template `<script>` is gone, pages are the sole emitter). Out-of-order responses can't apply stale head/JSON-LD: monotonic `loadSeq`/`fetchSeq` guards on product/brand/promotion detail, the product-list fetch, and the category-resolution layer. `index.html` gets the ClimaSite brand title + static OG/Twitter defaults (a real 1200×630 raster `og-default.png` — scrapers ignore SVG), and **B-033** sets `<html lang>` on first paint for returning BG/DE users. New translatable `seo.*` keys across en/bg/de. **A fatal `NG0200` circular DI (Router → TitleStrategy → Router) that silently broke ALL runtime SEO was caught only by the runtime `/acceptance` pass — every unit test was green** — fixed by removing `Router` from `SeoService`/`SeoTitleStrategy` (default canonical from `document.location.pathname`) and moving the NavigationStart JSON-LD reset into an app-initializer. **Wave B (backend):** a dynamic `~/robots.txt` (Allow `/`, Disallow the private SPA prefixes — but NOT `/api`, which Googlebot needs to render the app — + an absolute `Sitemap:` line) and a dynamic `~/sitemap.xml` (2285 URLs: static pages + all active product/brand slugs + active categories as `/products/category/{slug}` — the real route, not a soft-404 `/categories/{slug}` — + `IsCurrentlyActive` promotions, each with `<lastmod>` from `UpdatedAt`, slugs `Uri.EscapeDataString`-encoded). The absolute base URL is resolved host-injection-safely (config `Seo:SiteBaseUrl` first, validated absolute-https; a request-host fallback only in Development/Testing; **fail-closed 503 + no `Sitemap:` line in Staging/Production when unconfigured** — never emits canonical URLs from an untrusted host; `XForwardedHost` deliberately NOT enabled, nginx sets the real `Host $host`). The host-independent `(path,lastmod)` enumeration is cached in `IMemoryCache` while the absolute documents render per-request, and `[OutputCache(NoStore=true)]` opts out of the app's path-keyed base policy (a real cross-host cache-poisoning fix a test caught). `/api/*` responses carry `X-Robots-Tag: noindex` (the root crawler files do not); `nginx.conf.template` gains exact-match `= /robots.txt` / `= /sitemap.xml` proxy locations and JS-independent `X-Robots-Tag` blocks for the private SPA prefixes (re-declaring the inherited security headers). **M8:** the product-detail DTO now carries the already-computed curated `MetaTitle`/`MetaDescription` (the frontend consumes them as `metaTitle ?? name`). Designs cross-vendor-councilled to convergence (Codex `gpt-5.5`@`xhigh`, 3 design rounds); both diffs councilled (Wave A REWORK×2 → CLEAN incl. the NG0200 fix; Wave B APPROVE-WITH-CHANGES → fixed). `/acceptance` PASS on the real running app for both waves (EN+DE head inspection; live robots/sitemap/XML) at `.planning/acceptance/B-044-seo-waveA.md` + `…-waveB.md`. Frontend suite 1764 + backend Core 424 / Application 893 / Api 456 green; no DB migration. Full **DEC-SSR** crawlability (SSR/prerender, non-JS-scraper OG fidelity, hreflang) remains a separate owner decision; unit-plan at `.planning/units/B-044-seo/unit-plan.md`.
